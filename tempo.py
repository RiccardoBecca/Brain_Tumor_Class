...
from utils import init_weights, TranslateEnergies, summary
...
def Train_routine(net, train_set, val_set, optimizer, wandb_flag, schedular, save_model_time, path_to_save, args):
    for epoch in range(args.numepochs):
        ...
        for batch_idx, batch in enumerate(tqdm(train_set)):
            ...
            scores = scores.reshape(len(batch),int(net.SN.num_nodes),3)
            ...
            loss.backward()
            if args.clip_norm==1:
                torch.nn.utils.clip_grad_norm_(parameters=net.SN.parameters(), max_norm=args.clip_value, norm_type=2.0)
                #torch.nn.utils.clip_grad_norm_(parameters=net.gammas, max_norm=args.clip_value, norm_type=2.0)
            else:
                torch.nn.utils.clip_grad_value_(parameters=net.SN.parameters(), clip_value=args.clip_value)
                #torch.nn.utils.clip_grad_value_(parameters=net.gammas, clip_value=args.clip_value)
            optimizer.step()
            #optimizer.zero_grad()
    ...
    torch.save(net.state_dict(), os.path.join(path_to_save, f"tm_{args.numepochs}.pt"))
...
def main():
    ...
    path=f"./trained_model/lay={args.layers}_lmax={args.lmax}_mul={args.mul}_lr={args.lr}_batchsize={args.batchsize}_epochs={args.numepochs}_optim={args.optim}_seed={args.seed}_clip_norm={args.clip_norm}_clip_value={args.clip_value}_gamma={args.gamma}_percent={args.percent}_cutoff={args.cutoff}_wd={args.wd}"
    ...
    if args.wandb==True:
        ...
        wandb.init(project="my-Alvis-Average-biased", config={**net_params, **{"learningrate":args.lr, "batchsize":args.batchsize,"numepochs":args.numepochs, "seed":args.seed, "optim":args.optim, "wd":args.wd}})
        
    net_r = RSN(net)
    net_r.to(device)
    net_r.apply(init_weights)

    if args.optim=="Adam":
        #optimizer = Adam([{'params':net_r.SN.parameters()}, {'params':net_r.gammas, 'lr':1e-4}], lr=args.lr, weight_decay=args.wd)
        #base_opt = torch.optim.Adam(net_r.SN.parameters(), lr=args.lr, weight_decay=args.wd)
        #opt = torchcontrib.optim.SWA(base_opt, swa_start=(args.numepochs-100)*len(train_set), swa_freq=1, swa_lr=1e-6)
        optimizer = Adam(net_r.SN.parameters(), lr=args.lr, weight_decay=args.wd)
    if args.optim=="RMSprop":
        optimizer = RMSprop([{'params':net_r.SN.parameters()}, {'params':net_r.gammas, 'lr': 1e-4}], lr=args.lr, weight_decay=args.wd)
    if args.optim=="AdamW":
        #optimizer = AdamW([{'params':net_r.SN.parameters()}, {'params':net_r.gammas, 'lr':1e-4}], lr=args.lr, weight_decay=args.wd)
        optimizer = AdamW(net_r.SN.parameters(), lr=args.lr, weight_decay=args.wd)
    #opt.optimizer.zero_grad()
    schedular = ReduceLROnPlateau(optimizer, "min", factor=args.gamma, patience=args.patience, cooldown=args.cooldown, verbose=True, min_lr=1e-6) #maybe it could be I do not have to insert opt but opt.optimizer

    Train_routine(net_r, train_set, val_set, optimizer, args.wandb, schedular, args.save_model, path, args)
